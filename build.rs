    ...
    match target_arch {
        ...
        "sparc" => {
            ...
            if !version.probe(84, 2024, 11, 10) || needs_target_feature_fallback(&version, None) {
                ...
                let mut v9 = false;
                if let Some(cpu) = target_cpu() {
                    // https://github.com/llvm/llvm-project/blob/llvmorg-20.1.0-rc1/llvm/lib/Target/Sparc/Sparc.td
                    match &*cpu {
        ...
        "m68k" => {
            ...
            if !version.probe(85, 2024, 12, 15) || needs_target_feature_fallback(&version, None) {
                ...
                if let Some(cpu) = target_cpu() {
                    // https://github.com/llvm/llvm-project/blob/llvmorg-20.1.0-rc1/llvm/lib/Target/M68k/M68k.td
                    match &*cpu {
...
mod version {
    ...
    impl Version {
        ...
        // https://github.com/rust-lang/rust/blob/1.84.0/src/bootstrap/src/core/build_steps/llvm.rs#L617
        pub(crate) const LATEST: Self = Self::stable(84, 18);
