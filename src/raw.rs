...
unsafe fn atomic_memcpy_aligned<const IS_LOAD: bool, T>(
    ...
    if const_hint!({ mem::align_of::<T>() >= 8 }) {
    ...
    if const_hint!({ mem::align_of::<T>() >= 4 }) {
    ...
    if const_hint!({ mem::align_of::<T>() >= 2 }) {
...
pub unsafe fn atomic_load_memcpy<T>(
    ...
    let Some(size) = const_hint!({ NonZeroUsize::new(mem::size_of::<T>()) }) else { return };
    ...
    #[cfg(not(target_arch = "avr"))] // AVR's actual register size is 8-bit.
    if const_hint!({ mem::align_of::<T>() < mem::size_of::<RegSize>() })
        && (
            ...
            // 2      ^ ^ ^ ^
            // 3        ^ ^ ^ ^
            const_hint!({ mem::size_of::<T>() >= mem::size_of::<RegSize>() * 2 })
                || count.get() >= const_hint!({ mem::size_of::<RegSize>() * 2 })
        )
    {
        ...
        }
        {
            ...
            unsafe {
                ...
                dst = dst.add(count.get() * mem::size_of::<RegSize>());
            }
        }
        ...
        )
    {
        ...
        }
        {
            ...
            unsafe {
                ...
                src = src.add(count.get() * mem::size_of::<RegSize>());
            }
        }
        if let Some(count) = NonZeroUsize::new(last.len()) {
