...
#[derive(Clone, Copy)]
#[repr(transparent)]
pub struct Dependency(DependencyType);
cfg_has_fast_consume! {
    ...
    impl Dependency {
        const NO_DEP: Self = Self(MaybeUninit::new(0));
    }
    impl<T> Dependent<T> {
        #[inline(always)]
    ...
    impl ops::BitOr for Dependency {
        ...
        fn bitor(self, rhs: Self) -> Self::Output {
            Self(arch::dep_with_dep(self.0, rhs.0))
        }
    }
    ...
    impl<T> WithDependency for &T {
        ...
        fn with_dep(self, dep: Dependency) -> Self {
            ...
            unsafe {
                &*(arch::ptr_with_dep(self as *const T as *mut u8, dep.0) as *const T)
            }
        }
    }
    impl<T> WithDependency for &mut T {
        ...
        fn with_dep(self, dep: Dependency) -> Self {
            // SAFETY: ptr_with_dep doesn't modify the actual address.
            unsafe {
                &mut *(arch::ptr_with_dep((self as *mut T).cast::<u8>(), dep.0).cast::<T>())
            }
        }
    }
    impl<T> WithDependency for *const T {
        #[inline(always)]
        fn with_dep(self, dep: Dependency) -> Self {
            arch::ptr_with_dep(self as *mut u8, dep.0) as *const T
        }
    }
    impl<T> WithDependency for *mut T {
        #[inline(always)]
        fn with_dep(self, dep: Dependency) -> Self {
            arch::ptr_with_dep(self.cast::<u8>(), dep.0).cast::<T>()
        }
    }
    impl<T> WithDependency for NonNull<T> {
        ...
        fn with_dep(self, dep: Dependency) -> Self {
            ...
            unsafe {
                Self::new_unchecked(
                    arch::ptr_with_dep(self.as_ptr().cast::<u8>(), dep.0).cast::<T>()
                )
            }
        }
    }
