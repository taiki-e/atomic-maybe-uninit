    ...
    ($test_name:ident, $ty:ident) => {
        ...
        fn $test_name() {
            unsafe {
                ...
                assert_eq!(x.load(Ordering::Relaxed).assume_init(), [!0; 5]);
                let x = PerByteAtomicMaybeUninit::<[$ty; 100]>::from([0; 100]);
                assert_eq!(x.load(Ordering::Relaxed).assume_init()[..], [0; 100][..]);
                x.store(MaybeUninit::new([!0; 100]), Ordering::Relaxed);
                assert_eq!(x.load(Ordering::Relaxed).assume_init()[..], [!0; 100][..]);
            }
        }
    };
    ...
    macro_rules! test_small_alignment {
        ($ty:ident) => {
            unsafe {
                ...
                assert_ne!(a.as_ptr() as usize % mem::align_of::<AtomicUsize>(), 0);
