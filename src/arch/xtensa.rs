        ...
        impl AtomicMemcpy for $ty {
            load_memcpy! { $ty, |src, tmp0, tmp1|
                asm!(
                    ...
                    concat!("addi.n {src}, {src}, ", $size),                           // src = src.byte_add($size)
                    src = inout(reg) src,
                    tmp0 = out(reg) tmp0,
                    options(nostack, preserves_flags),
                ),
                asm!(
                    ...
                    concat!("l", $bits, $unsigned, "i", $narrow, " {tmp0}, {src}, 0"),       // atomic { tmp0 = *src }
                    concat!("addi.n {src}, {src}, 2*", $size),                               // src = src.byte_add(2*$size)
                    src = inout(reg) src,
                    tmp0 = out(reg) tmp0,
                    tmp1 = out(reg) tmp1,
                    options(nostack, preserves_flags),
            ...
            store_memcpy! { $ty, |dst, tmp0, tmp1|
                asm!(
                    ...
                    concat!("addi.n {dst}, {dst}, ", $size),                // dst = dst.byte_add($size)
                    dst = inout(reg) dst,
                    tmp0 = in(reg) tmp0,
                    options(nostack, preserves_flags),
                ),
                asm!(
                    ...
                    concat!("s", $bits, "i", $narrow, " {tmp0}, {dst}, 0"),       // atomic { *dst = tmp0 }
                    concat!("addi.n {dst}, {dst}, 2*", $size),                    // dst = dst.byte_add(2*$size)
                    dst = inout(reg) dst,
                    tmp0 = in(reg) tmp0,
                    tmp1 = in(reg) tmp1,
                    options(nostack, preserves_flags),
