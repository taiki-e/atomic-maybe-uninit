...
impl AtomicMemcpy for u64 {
    ...
    store_memcpy_opt64! { |dst, tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7|
        asm!(
            "std {tmp0}, 0({dst})", // atomic { *dst = tmp0 }
        ...
        asm!(
            "std {tmp1}, 8({dst})",     // atomic { *dst.byte_add(8) = tmp1 }
            "std {tmp0}, 0({dst})",     // atomic { *dst = tmp0 }
        ...
        asm!(
            "std {tmp7}, 8*7({dst})",   // atomic { *dst.byte_add(8*7) = tmp7 }
            "std {tmp6}, 8*6({dst})",   // atomic { *dst.byte_add(8*6) = tmp6 }
            "std {tmp5}, 8*5({dst})",   // atomic { *dst.byte_add(8*5) = tmp5 }
            "std {tmp4}, 8*4({dst})",   // atomic { *dst.byte_add(8*4) = tmp4 }
            "std {tmp3}, 8*3({dst})",   // atomic { *dst.byte_add(8*3) = tmp3 }
            "std {tmp2}, 8*2({dst})",   // atomic { *dst.byte_add(8*2) = tmp2 }
            "std {tmp1}, 8*1({dst})",   // atomic { *dst.byte_add(8*1) = tmp1 }
            "std {tmp0}, 0({dst})",     // atomic { *dst = tmp0 }
