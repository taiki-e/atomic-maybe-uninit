...
macro_rules! within_register {
    ($ty:ident) => {
        // TODO(bpf): use bpf_core_type_size once https://github.com/aya-rs/aya/issues/349 implemented?
        mem::size_of::<$ty>() <= 4
    };
}
    ...
    ($ty:ident, $reg:ident, $r0:tt, $cmpxchg:tt, $xchg:tt, $reg_in:ident $($reg_in_out:tt)*) => {
        ...
        impl AtomicLoad for $ty {
            ...
            unsafe fn atomic_load(
                ...
                unsafe {
                    const WITHIN_REGISTER: bool = within_register!($ty);
                    match order {
                        Ordering::Relaxed if WITHIN_REGISTER => {
                            asm!(
