                ...
                unsafe {
                    ...
                    asm!(
                        ...
                        "2:", // 'retry:
                            ...
                            concat!("risbg {out}, {val}, 32, 31+", $size, "*8, 32-", $size, "*8"), // out = out | (val.rotate_left(32 - $size * 8) & ((!0$ty as u32) << (32 - $size * 8)))
                            "rll {out}, {out}, 0({shift_c})",                                      // out = out.rotate_left(shift_c)
                            "cs {prev}, {out}, 0({dst})",                                          // atomic { if *dst == prev { cc = 0; *dst = out } else { cc = 1; prev = *dst } }
                            "jl 2b",                                                               // if cc == 1 { jump 'retry }
                        concat!("rll {out}, {prev}, ", $size, "*8({shift})"),                      // out = prev.rotate_left(shift + $size * 8)
                        dst = in(reg_addr) ptr_reg!(dst),
                        val = in(reg) val,
                        out = out(reg) out,
            ...
            unsafe fn atomic_compare_exchange(
                ...
                unsafe {
                    ...
                    asm!(
                        ...
                        "2:", // 'retry:
                            concat!("rll {out}, {prev}, ", $size ,"*8({shift})"),   // out = prev.rotate_left(shift + $size * 8)
                            concat!("risbg {new}, {out}, 32, 63-", $size, "*8, 0"), // new = (new & !(!0$ty as u32)) | (out & !0$ty as u32)
                            ...
                            concat!("rll {tmp}, {new}, -", $size ,"*8({shift_c})"), // out = prev.rotate_left(shift_c - $size * 8)
...
atomic_sub_word!(u8, "1", "b", "c");
atomic_sub_word!(u16, "2", "h", "h");
atomic!(u32, "4", "");
atomic!(u64,    , "g");
