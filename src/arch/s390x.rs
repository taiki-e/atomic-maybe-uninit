                ...
                unsafe {
                    ...
                    asm!(
                        ...
                        "2:", // 'retry:
                            ...
                            concat!("risbg {out}, {val}, 32, ", $risbg_swap), // out = risbg(out, val, 32, $risbg_swap)
                            "rll {out}, {out}, 0({shift_c})",                 // out = out.rotate_left(shift_c)
                            "cs {prev}, {out}, 0({dst})",                     // atomic { if *dst == prev { cc = 0; *dst = out } else { cc = 1; prev = *dst } }
                            "jl 2b",                                          // if cc == 1 { jump 'retry }
                        concat!("rll {out}, {prev}, ", $bits ,"({shift})"),   // out = (prev >> shift) & $ty::MAX
                        dst = in(reg_addr) ptr_reg!(dst),
                        val = in(reg) val,
                        out = out(reg) out,
            ...
            unsafe fn atomic_compare_exchange(
                ...
                unsafe {
                    ...
                    asm!(
                        ...
                        "2:", // 'retry:
                            concat!("rll {out}, {prev}, ", $bits ,"({shift})"),     // out = (prev >> shift) & $ty::MAX
                            concat!("risbg {new}, {out}, 32, ", $risbg_cas, ", 0"), // TODO
