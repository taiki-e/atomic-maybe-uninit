...
impl<'a> Rustflags<'a> {
    pub(crate) fn new(rustflags: &'a str, cargo_unstable_allow_features: Option<&'a str>) -> Self {
        ...
        while let Some(mut flag) = rustflags.next() {
            ...
            if flag.as_bytes().get(start) == Some(&b'-') {
                ...
                while matches!(flag.as_bytes().get(start), Some(b'g' | b'O' | b'v')) {
                    start += 1;
                }
            }
            let Some(&b) = flag.as_bytes().get(start) else { continue };
            match b {
                // -C / -Z
                b'C' | b'Z' => {
                    start += 1;
                    if flag.len() == start {
                        let Some(f) = rustflags.next() else { break };
                        flag = f;
                    } else if long {
                        if flag.as_bytes().get(start) != Some(&b'=') {
                            continue;
                ...
                _ => {
                    ...
                    if let Some(f) = flag.strip_prefix("codegen") {
